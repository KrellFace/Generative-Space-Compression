        ...
        for char in charlist:
            ...
            if (char == '\n'):
                ...
                #Check if we are at the end of a level rep. If we are, add it to our dictionary
                    ...
                    new_level = BoxobanLevel(temp_levelname, folder_name,  helper.get_n_last_subparts_path(file_path, 2), char_matrix)
...
def get_leveldict_from_folder(path, folder_key, game):
    file_names = helper.get_filenames_from_folder(path)
...
def get_leveldicts_from_folder_set(game):
    level_dict = dict()
    game_info = helper.get_folder_and_tiletypedict_for_game(game)
...
def get_randnum_levelwrappers_boxoban(folders_dict, maxlvlsevaled):
    ...
    for folder in folders_dict:
        ...
        file_list = helper.get_filenames_from_folder(folders_dict[folder])
