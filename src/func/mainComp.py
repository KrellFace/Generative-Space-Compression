...
def multigenerator_compression(levelwrapper_dict, game, comp_algo, component_count = 2, visualise = False, plot_filename_root = ""):
    ...
    print(f"DR Baseline Var Exp: {compressed_info[1]}")
    if visualise == True:
        plot_filename = f"{plot_filename_root}_{comp_algo.name}.png"
...
def get_and_update_X_levels_for_algo_list(game, component_count, algolist, maxlvlsevaled, visualise = False, file_root = ""):
    ...
    for algo in algolist:
        print(f"Running algo : {algo.name}")
        ...
        print(f"Algo {algo.name} compression runtime: {datetime.now() -start_time} seconds")
...
def gen_compression_dist_df_from_leveldict(level_wrapper_dict, game, algolist, bclist, output_filepath):
    ...
    for algo in algolist:
        nearfar_exemplar_dict[f"{algo.name} Closest"] = [-1, 10000 ,None, None]
        nearfar_exemplar_dict[f"{algo.name} Furthest"] = [-1, 0,None, None]
    ...
    for pair in uniquepairs:
       ...
       # print("BC Vals: " + str(bc_vals_list))
        ...
        for i in range(0,len(algolist)):
            if (algo_dist_list[i]<nearfar_exemplar_dict[f"{algolist[i].name} Closest"][1]):
                nearfar_exemplar_dict[f"{algolist[i].name} Closest"] = [pair_counter, algo_dist_list[i], level1, level2]
            if (algo_dist_list[i]>nearfar_exemplar_dict[f"{algolist[i].name} Furthest"][1]):
                nearfar_exemplar_dict[f"{algolist[i].name} Furthest"] = [pair_counter, algo_dist_list[i], level1, level2]
        
        pair_counter+=1

        if (pair_counter%200000 == 0):
            print("200000 level pairs processed. Counter: " + str(pair_counter))
            print(f"Runtime: {datetime.now () -start_time} seconds")
    ...
    for exemp in nearfar_exemplar_dict:
        ...
        lvl1outputpath = f"{output_filepath}{exemp} Level1.png"
        lvl2outputpath = f"{output_filepath}{exemp} Level2.png"
...
def multidomain_multiruns(games, component_count, algolist, tot_lvls_evaled_per_run, runs_per_game, file_prefix, visualise = False):
    ...
    for game in games:
        ...
        while runcount < runs_per_game:
            runpath = f"{file_prefix}/{game.name}/Run {runcount+1}/"
            ...
            lincorrsfilepath = Path(f"{runpath}{game.name} Run {runcount+1}.txt")
            ...
            for key in temp_corrsdict:
                ...
                lincorrdict[f"{game.name} {runcount}{key}"] = linncorrs
            runcount += 1

            runtime_seconds=  datetime.now () -start_time
            runtime_minutes = runtime_seconds/60
            print(f"Runtime: {runtime_minutes} minutes at run {runcount} for game: {game.name}")
    ...
    curr_time = datetime.now().strftime("%m_%d_%H_%M_%S")
    finaloutputpath = Path(f"{file_prefix}/Total Lin Corrs.csv")
